...
def do(symbol , timeframe , limit , params:dict):
    ...
    start = time.time()
    for t in symbol:
        ohlcv = bybit.fetch_ohlcv(t,timeframe=timeframe,limit=limit*3)
...
def delay():
    s = time.time()
    _ = bybit.fetch_ticker('ADA/USDT:USDT')['last']
    e = time.time()
    return f"{e-s:.3f}s"

def sorted_by_trades(symbol_list):
    symbol_list.sort(key=lambda x:(bybit.fetch_ohlcv(x,timeframe='1d',limit=2)[0][-1])*sum(bybit.fetch_ohlcv(x,timeframe='1d',limit=2)[0][1:-1:])/4,reverse=True)
    return symbol_list
...
if __name__ == '__main__':
    ...
    bybit.load_markets()
    symbol_list = symbol_list.split(' ') if symbol_list != 'ALL' else [t for t in bybit.symbols if t.endswith(':USDT')]
    print(f'總共有{len(symbol_list)}個幣種，正在排序中...')
    rank = int(input('請輸入排名:'))
    symbol_list = sorted_by_trades(symbol_list)[:rank]
    print(f'排序完成!')
    ...
    print('================')
    print('系統狀態: 啟動完成')
    schedule.every().day.at("08:15").do(daily_reflash)#add this
    schedule.every(timeframe).minutes.at(":01").do(main,symbol_list=symbol_list,timeframe=timeframe,length=length,params={'smooth_length': smooth_length,'over_buy': over_buy,'over_sell': over_sell,'timeframe': timeframe,'keyword':keyword,'rank':rank})
    while True:
